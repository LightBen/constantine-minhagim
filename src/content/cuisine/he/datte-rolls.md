---
title: "גלילי תמרים"
locale: "he"
flamelinkId: "sGUp59YMKKp03bY4So6Y"
date: "2020-01-06T16:40:05.000Z"
updatedDate: "2020-03-03T18:40:23.000Z"
author: "קורלי כהן אוואט"
description: ""
banner: "https://res.cloudinary.com/constantineminhagim/image/upload/f_auto,q_auto,dpr_auto/v1583258794/cuisine/cuisine-banners/roules-sdattes_rwmlh2.jpg"
thumbnail: "https://res.cloudinary.com/constantineminhagim/image/upload/f_auto,q_auto,dpr_auto/v1583258752/cuisine/cuisine-thumbnails/roules-sdattes_mkzazw.jpg"
tags: []
videoUrl: ""
foodType: ["parve"]
dishType: ["gateau"]
events: ["autre", "sans", "pourim"]
---

<p><strong>מרכיבים</strong></p>

<ul>
	<li>קילו תמרים מעוכים (לקבלת 6 גלילים)</li>
	<li>קמצוץ קינמון</li>
	<li>קליפת תפוז מגוררת</li>
	<li>אבקת סוכר לקישוט</li>
	<li>לבצק:
	<ul>
		<li>400 גר&#39; קמח</li>
		<li>300 גר&#39; מרגרינה מומסת</li>
		<li>1/2 כוס מים</li>
		<li>1 שקית אבקת אפיה</li>
		<li>1 שקית סוכר וניל</li>
	</ul>
	</li>
</ul>

<p><strong>אופן ההכנה</strong></p>

<ol>
	<li>לערבב את כל החומרים לעשיית הבצק</li>
	<li>לחלק ל4 חלקים</li>
	<li>לרדד את הבצק, למרוח את התמרים מעל הבצק ולעצב את הגליל</li>
	<li>להוסיף את קליפת התפוז והקנמון</li>
	<li>לאפות בתנור (170 מעלות C)</li>
	<li>כשהגלילים התקררו, לפזר אבקת סוכר ולחתוך פרוסות.</li>
</ol>
